...
public class Particle {
	...
	public Particle(int x, int y) {
		...
		this.width = r.nextInt(MAX_DIMENSION);
		this.height = this.width;
		this.lifetime = DEFAULT_LIFETIME;
		this.age = 0;
		
		this.xv = (r.nextDouble() * (MAX_SPEED * 2) - MAX_SPEED);
		this.yv = (r.nextDouble() * (MAX_SPEED * 2) - MAX_SPEED);
		// smoothing out the diagonal speed
		if (xv * xv + yv * yv > MAX_SPEED * MAX_SPEED) {
			xv *= 0.7;
			yv *= 0.7;
		}
		this.color = Color.argb(255, r.nextInt(255), r.nextInt(255), r.nextInt(255));
